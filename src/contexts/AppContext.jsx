...
const loadFromStorage = () => ({
    ...
    userName: localStorage.getItem(STORAGE_KEYS.USERNAME) || 'Sampiyonlar',
    lessons: (() => {
...
export function AppProvider({ children }) {
  ...
  const pushTimeoutRef = useRef(null);
    ...
    // pendingPushRef: true iken sunucu verisinin local state'i ezmesi engellenir
  const pendingPushRef = useRef(false);
    // lastPushTimestamp: son basarili push'un zamani - poll bu zamandan onceki veriyi ignore eder
  ...
  const getSyncPayload = useCallback(() => ({
        ...
        recipes,
        mealLogs,
  }), [userName, theme, userStats, leftLayout, rightLayout, habits, tasks, taskLogs, goals,
           lessons, lessonTemplates, students, expenses, waterLogs, coffeeLogs, workoutLogs, recipes, mealLogs]);

  // Mount'ta sunucudan veri cek
  useEffect(() => {
        if (initialFetchDone.current) return;
        initialFetchDone.current = true;
        setSyncStatus('loading');
        const initialPayload = {
                userName: stored.userName, theme: stored.theme,
                userStats: { xp: stored.userStats.xp, level: stored.userStats.level },
                leftLayout: stored.leftLayout, rightLayout: stored.rightLayout,
                habits: stored.habits, tasks: stored.tasks, taskLogs: stored.taskLogs,
                goals: stored.goals, lessons: stored.lessons, lessonTemplates: stored.lessonTemplates,
                students: stored.students, expenses: stored.expenses, waterLogs: stored.waterLogs,
                coffeeLogs: stored.coffeeLogs, workoutLogs: stored.workoutLogs,
                recipes: stored.recipes, mealLogs: stored.mealLogs,
        };
        fetchSync()
          .then((data) => {
                    const serverHadData = (
                                data.habits?.length || data.tasks?.length || data.lessons?.length ||
                                data.lessonTemplates?.length || data.students?.length || data.expenses?.length ||
                                Object.keys(data.waterLogs || {}).length || Object.keys(data.coffeeLogs || {}).length ||
                                data.workoutLogs?.length || data.recipes?.length || data.mealLogs?.length || data.goals?.length
                              ) > 0;
                    const hasLocalData = (
                                initialPayload.habits?.length || initialPayload.tasks?.length ||
                                initialPayload.lessons?.length || initialPayload.lessonTemplates?.length ||
                                initialPayload.students?.length || initialPayload.expenses?.length ||
                                Object.keys(initialPayload.waterLogs || {}).length ||
                                Object.keys(initialPayload.coffeeLogs || {}).length ||
                                initialPayload.workoutLogs?.length || initialPayload.recipes?.length ||
                                initialPayload.mealLogs?.length || initialPayload.goals?.length
                              ) > 0;
                    if (serverHadData) {
                                applyServerData(data);
                    } else if (hasLocalData) {
                                pushSync(initialPayload)
                                  .then(() => { lastPushTimestampRef.current = Date.now(); })
          ...
          .catch((err) => {
                    ...
                    const hasLocalData = initialPayload.habits?.length || initialPayload.tasks?.length ||
                                initialPayload.lessons?.length || initialPayload.lessonTemplates?.length ||
                                initialPayload.students?.length || initialPayload.expenses?.length;
                    if (hasLocalData) {
                                pushSync(initialPayload)
                                  .then(() => { setSyncStatus('synced'); setSyncError(null); lastPushTimestampRef.current = Date.now(); })
                                  .catch(() => {});
                    }
                    initialFetchCompletedRef.current = true;
          });
  ...
  // Sekme kapanirken / arka plana giderken hemen push et
  useEffect(() => {
        const onHide = () => {
                ...
                pushSync(getSyncPayload())
                  .then(() => { pendingPushRef.current = false; lastPushTimestampRef.current = Date.now(); })
  ...
  // Periyodik polling: 30 saniyede bir - daha az agresif, race condition riski dusuk
  // FIX: pendingPushRef veya push zamanindan sonraki GRACE_PERIOD_MS icerisinde overwrite etme
  const POLL_INTERVAL_MS = 30_000;
    const GRACE_PERIOD_MS = 5_000; // Push'tan sonra 5 saniye bekle

  useEffect(() => {
        if (!initialFetchDone.current) return;
        const doFetch = () => {
                // Bekleyen push varsa veya son push'tan itibaren grace period dolmadiysa overwrite etme
                if (pendingPushRef.current) return;
                if (Date.now() - lastPushTimestampRef.current < GRACE_PERIOD_MS) return;

                fetchSync()
                  .then((data) => {
                              // Tekrar kontrol et - fetch sirasinda push baslamis olabilir
                                  if (pendingPushRef.current) return;
                              if (Date.now() - lastPushTimestampRef.current < GRACE_PERIOD_MS) return;
        ...
        const onVisibilityChange = () => { if (document.visibilityState === 'visible') doFetch(); };
        document.addEventListener('visibilitychange', onVisibilityChange);
  ...
  }, [applyServerData]); // eslint-disable-line react-hooks/exhaustive-deps

  // Degisiklikleri sunucuya push et (debounced 500ms)
  // FIX: 300ms -> 500ms, poll 30s oldugu icin race condition yok ama safety icin arttirildi
  ...
  useEffect(() => {
        ...
        return () => { if (pushTimeoutRef.current) clearTimeout(pushTimeoutRef.current); };
          ...
          waterLogs, coffeeLogs, workoutLogs, recipes, mealLogs, userName, theme, userStats,
          leftLayout, rightLayout, schedulePush]);

  // localStorage'a kaydet (offline fallback)
  ...
  const value = {
        ...
        lessons, setLessons,
        lessonTemplates, setLessonTemplates,
        students, setStudents,
        expenses, setExpenses,
        waterLogs, setWaterLogs,
        coffeeLogs, setCoffeeLogs,
        workoutLogs, setWorkoutLogs,
        recipes, setRecipes,
        mealLogs, setMealLogs,
        leftLayout, setLeftLayout,
        rightLayout, setRightLayout,
  ...
  return <AppContext.Provider value={value}>{children}</AppContext.Provider>AppContext.Provider>;
}
